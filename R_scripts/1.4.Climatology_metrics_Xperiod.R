...
###                                                             ################
### 1.4- Transform yearly seasonal metrics into climatology metrics   ##########
...
source(paste("/R_scripts/functions","RFuntion_merge_nemo_climatology_data.R", sep="/"))

variables<- c("current_speed") 
...
output_path<- "/modeled_variables_original/climatology_metrics_post_blob"
...
for (variable in variables) {
  ...
  for (season in seasons) {
    ...
    for (metric in metrics) {
      ...
      grid_template <- read.csv("/ubcSSnBathymetryV21-08_d0bf_13e6_3a75.csv") 
      ...
      merged_data_mean <- merge_nemo_climatology_data(
        ...
        variable = variable,
        metric = metric,
        input_path = "/modeled_variables_original/seasonal_metrics_0.5m_depth",
        season = season,
...
#### Calculate temperature above threshold metrics ===============
output_path<- "/Volumes/Romina_PSF/PSF/modeled_variables_original/climatology_temp_tolerance_metrics"
setwd("/modeled_variables_original/seasonal_metrics_0.5m_depth")
dir()
setwd(output_path)

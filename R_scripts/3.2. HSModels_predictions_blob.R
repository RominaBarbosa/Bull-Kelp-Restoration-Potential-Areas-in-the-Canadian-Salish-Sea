...
# Save rasters
terra::writeRaster(ens_out$raster, "tifs/ensemble_wAverage_suitability_blob.tif", overwrite=TRUE)
terra::writeRaster(ens_out_normalAverage$raster, "tifs/ensemble_Average_suitability_blob.tif", overwrite=TRUE)
# writeRaster(ensemble_SD$max, "tifs/ensemble_Ave_model_max.tif")
# writeRaster(ensemble_SD$min, "tifs/ensemble_Ave_model_min.tif")
# writeRaster(ensemble_SD$sd, "tifs/ensemble_Ave_SD_blob.tif")
# writeRaster(ensemble_SDweithed$max, "tifs/ensemble_wAve_model_max.tif")
# writeRaster(ensemble_SDweithed$min, "tifs/ensemble_wAve_model_min.tif")
# writeRaster(ensemble_SDweithed$sd, "tifs/ensemble_wAve_SDweithed_blob.tif")


ens_average<- terra::rast("tifs/ensemble_Average_suitability_blob.tif")
ens_wAverage<- terra::rast("tifs/ensemble_wAverage_suitability_blob.tif") 
  
# Save weights + TSS table
write.csv(ensemble_SD$weights, "blob_model_tss_weights.csv")
...
substrate_aligned[substrate_aligned == 3]<- NA
substrate_aligned[substrate_aligned == 4]<- NA

# writeRaster(substrate_aligned, "tifs/substrate_SOG_aligned.tif", overwrite=T)

ens_average_masked <- terra::mask(ens_average, substrate_aligned)
ens_wAverage_masked <- terra::mask(ens_wAverage, substrate_aligned)
# writeRaster(ens_average_masked, "tifs/ens_average_masked.tif", overwrite=T)
# writeRaster(ens_wAverage_masked, "tifs/ens_wAverage_masked.tif", overwrite=T))


ens_average_masked_all<- rast("tifs/ens_average_masked.tif")
ens_wAverage_masked_all<- rast("tifs/ens_wAverage_masked.tif")
plot(ens_wAverage_masked_all)
plot(ens_average_masked_all)
...
train_masked<- train[,c(3,5,6,26,27)]
...
# write.csv(models_performance, "models_performance_calibration_Table.csv")
# Model         ThresholdType Threshold   AUC Sensitivity Specificity   TSS
...
# Thresholds
threshold_drop <- quantile(pres_pred, 0.001, na.rm = TRUE)  # 0.1% omission --> 0.2054824 
thr_maxTSS <- thr_maxTSS  # 0.6940952

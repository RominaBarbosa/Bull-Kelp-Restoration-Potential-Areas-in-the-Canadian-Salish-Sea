...
train_sel<- read.csv("train_selected_table_FINALMODELS.csv")
train_sel_scaled<- read.csv("train_selected_scaled_table_FINALMODELS.csv")
test_sel<- read.csv("test_selected_table_FINALMODELS.csv")
...
fviz_pca_biplot(pca_res,
                ...
                habillage = train_weight$cluster, # color by cluster
...
train.xy$Cluster<- train_weight$cluster
str(train.xy)
...
# -----------------------------
freq <- table(train_weight[which(train_weight$kelp==1), "cluster"])
inv_freq <- 1 / as.numeric(freq)
names(inv_freq) <- names(freq)

# Now lookup by cluster number as character
pres_weights <- inv_freq[as.character(train_weight$cluster[train_weight$kelp==1])]
...
# write.csv(cv_results, "Cvalidation_results_M5_v2.csv")
...
pred_df_sel <- bind_rows(
  data.frame(Probability = predict(glm_mod_se, newdata = test_scaled, type = "response"), Model = "GLM", Truth = test$kelp),
  data.frame(Probability = predict(gam_mod_se, newdata = test_scaled, type = "response"), Model = "GAM", Truth = test$kelp),
  data.frame(Probability = predict(rf_mod_se, newdata = test, type = "prob")[, 2], Model = "RF", Truth = test$kelp),
  data.frame(Probability = predict(brt_mod_se, newdata = test, n.trees = brt_mod_se$n.trees, type = "response"), Model = "BRT", Truth = test$kelp)
)
...
# ggsave("/Volumes/Romina_PSF/PSF/SDM/Variables_selection/plots/VariablesImportance_SelectedModels_M5.pdf", width = 11, height = 6, dpi= 300, units="cm")
# ggsave("/Volumes/Romina_PSF/PSF/SDM/Variables_selection/plots/VariablesImportance_SelectedModels_M5.png", width = 11, height = 6, dpi= 300, units="cm")


plot_importance_profiles(variablesImportance_models_selected$summary_table)
# ggsave("/Volumes/Romina_PSF/PSF/SDM/Variables_selection/plots/VariablesImportance_profile_selectedModels_Sep2025_M5.png", width = 18, height = 12, dpi= 300, units="cm")
# ggsave("/Volumes/Romina_PSF/PSF/SDM/Variables_selection/plots/VariablesImportance_profile_selectedModels_M5.pdf", width = 18, height = 12, dpi= 300, units="cm")
...
# Variables to plot
vars_to_plot <- names(train)[-1]  # all variables except response
...
# ggsave("/Volumes/Romina_PSF/PSF/SDM/Variables_selection/plots/ResponseCurves_SelectedVariables_M5.png", width = 19, height = 9, dpi= 300, units="cm")
# ggsave("/Volumes/Romina_PSF/PSF/SDM/Variables_selection/plots/ResponseCurves_SelectedVariables_M5.pdf", width = 19, height = 9, dpi= 300, units="cm")
...
# ggsave("/Volumes/Romina_PSF/PSF/SDM/Variables_selection/plots/ResponseCurves_SelectedVariables_average_Sep2025_M5.png", width = 19, height = 9, dpi= 300, units="cm")
# ggsave("/Volumes/Romina_PSF/PSF/SDM/Variables_selection/plots/ResponseCurves_SelectedVariables_average_Sep2025_M5.pdf", width = 19, height = 9, dpi= 300, units="cm")

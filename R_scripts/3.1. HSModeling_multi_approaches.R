...
kelp_points <- vect(kelp_presabs_df, geom = c("x", "y"), crs = "EPSG:3005")
plot(kelp_points, cex=0.5)
...
terrain_vars<-  rast(tif_files[c(2,5,12,15)])
...
raster_stack_20m_all<- c(raster_stack_20m, terrain_vars[[3:4]])
...
# # Explore incoherences
# na_counts <- colSums(is.na(extracted_values[,-1]))
# na_percent <- round(na_counts / nrow(extracted_values) * 100, 1)
# na_summary <- data.frame(
#   layer = names(raster_stack_20m_all),
#   NA_count = na_counts,
#   NA_percent = na_percent
# )
# print(na_summary[order(-na_summary$NA_percent), ])
# 
# ### 2️⃣ Identify points with NAs for the worst layer(s)
# worst_layer <- na_summary$layer[which.max(na_summary$NA_percent)]
# problem_points <- kelp_points[is.na(extracted_values[[worst_layer]]), ]
# 
# ### 3️⃣ Plot problem areas for inspection
# plot(raster_stack_20m_all[[worst_layer]], main = paste("Problem Layer:", worst_layer))
# points(problem_points, col = "red", pch = 20, cex = 0.6)
# points(kelp_points, col = "blue", pch = 3, cex = 0.5)
# legend("bottomleft",
#        legend = c("All kelp points", "NA points"),
#        col = c("blue", "red"), pch = c(3, 20))
...
kelp_data_with_variables%>%
  group_by(kelp)%>%
  summarize(n= length(kelp), depth_max= max(bathymetry_20, na.rm = T), 
            depth_min= min(bathymetry_20, na.rm = T))
...
kelp_data_with_variables<- kelp_data_with_variables%>%
  filter(bathymetry_20 >= -10)%>%
  filter(bathymetry_20 <= 40)
...
kelp_data_with_variables<- kelp_data_with_variables%>%
  filter(!is.na(TPI_3x3))

summary(kelp_data_with_variables)

# write.csv(kelp_data_with_variables, "/Volumes/Romina_PSF/PSF/SDM/Presence_absences_kelp/Presence_absences_kelp_variablesSelectedFINAL2.csv")
...
df<- read.csv("/Volumes/Romina_PSF/PSF/SDM/Presence_absences_kelp/Presence_absences_kelp_variablesSelectedFINAL2.csv")
df<- df[,-1]
colnames(df)
...
quantile(presences$bathymetry_20, 0.9) #26.51765
quantile(presences$bathymetry_20, 0.95) #32.6647
quantile(presences$bathymetry_20, 0.93) #30.14825 
quantile(presences$bathymetry_20, 0.99) #37.68245
...
# write.csv(train_test_dataset, "/Volumes/Romina_PSF/PSF/SDM/SDM_results/training_testing_datasets_blob_FINAL2.csv")
...
predictors <- setdiff(
  ...
  c("kelp", "cell_id", "env_cluster", "bathymetry_20","x", "y", "set", "cell", "easterness_3x3",           "northerness_3x3" 
...
quad_vars <- colnames(train_scaled)[2:10]
...
# rf_mod <- randomForest(as.formula(paste("kelp ~", paste(predictors, collapse = " + "))),
...
train_brt <- train# %>% select(-env_cluster)
colnames(train_brt)
...
# ggsave("/Volumes/Romina_PSF/PSF/SDM/Variables_selection/plots/ResponseCurves_allVariables_FINAL.png", width = 16, height = 17, dpi= 300, units="cm")


#-----------------------------
# Collect results
results <- bind_rows(
  get_metrics_optimized(glm_mod_quad, test_scaled, "glm"),
  get_metrics_optimized(gam_mod, test_scaled, "gam"),
  get_metrics_optimized(rf_mod, test, "rf"),
  get_metrics_optimized(brt_mod, test, "brt")
)
...
# ggsave("/Volumes/Romina_PSF/PSF/SDM/Variables_selection/plots/VariablesImportance_profile_step_1_FINAL2.png", width = 18, height = 12, dpi= 300, units="cm")
# ggsave("/Volumes/Romina_PSF/PSF/SDM/Variables_selection/plots/VariablesImportance_profile_step_1_FINAL2.pdf", width = 18, height = 12, dpi= 300, units="cm")
...
# After removing SD metrics and easterness and northerness
...
# ==============================================================================
setwd("/Volumes/Romina_PSF/PSF/SDM/SDM_results")
thresh= 15
...
# save.image("/Volumes/Romina_PSF/PSF/SDM/SDM_results/results_SDM_Sep2025.RData")
...
results_selected <- bind_rows(
  get_metrics_optimized2(model= glm_mod_s, test_data=test_sel, scale_params= scaling_params_2,
...
# write.csv(results_selected, "models_performance_Table3_thresholds_Sep2025.csv")
